...
def main():
    ...
    existing_voice_pipeline = st.session_state.get("voice_pipeline")
    if getattr(existing_voice_pipeline, "version", None) != VoicePipeline.VERSION:
        try:
            ...
            groq_client = (
                Groq(api_key=api_key, timeout=8.0, max_retries=1)
                if api_key
                else None
            )
            ...
            st.session_state.voice_mode = "Groq AI" if api_key else "Built-in fallback"
            st.session_state.voice_setup_error = None
        except Exception as e:
            st.session_state.voice_pipeline = None
            st.session_state.voice_mode = "Unavailable"
            st.session_state.voice_setup_error = str(e)
    ...
    with st.sidebar:
        ...
        if not workout_started:
            ...
            if end_session_button:
                ...
                already_announced = st.session_state.get(
                    "last_notified_workout_complete", False
                )

                if st.session_state.voice_pipeline and not already_announced:
                    result = st.session_state.voice_pipeline.process_event(
    ...
    voice_error = st.session_state.get("voice_setup_error")
    if voice_pipeline and getattr(voice_pipeline, "last_error", None):
        voice_error = voice_pipeline.last_error

    if voice_error:
        st.warning(f"Voice coach error: {voice_error}")
